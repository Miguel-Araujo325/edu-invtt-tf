...
async function carregarAgendamentos(pagina) {
    ...
    try {
        ...
        const resposta = await fetch(`http://98.81.188.68/api/agendamento/historico/${id}?page=${pagina}&tempo=${tempo}` + Filters.buildQueryString(), {
            method: 'GET',
            headers: {
...
async function buscarDetalhes(id) {
    ...
    const respostaHistorico = await fetch(`http://98.81.188.68/api/historico-agendamento/${id}`, {
        method: 'GET',
        headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
...
async function novoStatus(id, statusId) {
    try {
        ...
        const respostaStatus = await fetch(`http://98.81.188.68/api/status/${statusId}`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
...
async function novoAssunto(id, assunto) {
    try {
        const novoStatus = await fetch(`http://98.81.188.68/api/agendamento/${id}`, {
            method: "PUT",
            body: assunto,
            headers: { 'Authorization': `Bearer ${token}`, "Content-type": "application/json; charset=UTF-8" }
        });
...
async function novoStatusTransferencia(idAgendamento, novoProfessorId) {
    try {
        const resposta = await fetch(`http://98.81.188.68/api/agendamento/transferir`, {
            method: "POST",
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
...
async function buscarProfessoreDisponveis() {
    ...
    try {
        const resposta = await fetch("http://98.81.188.68/api/horario-professor/transferencia", {
            method: "POST",
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
