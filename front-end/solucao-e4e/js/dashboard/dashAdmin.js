...
const meta = async (id) => {
    try {
        const response = await fetch(`http://54.159.90.230/api/metas/usuario/${id}`, {
            method: 'GET',
            headers: {
...
async function plotarKPIsProfessor() {
    try {
        const responseAgendamentos = await fetch(`http://54.159.90.230/api/dashboard/qtd-agendamento-mes-professor/${id}`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${sessionStorage.getItem('token')}`,
                'Content-Type': 'application/json'
