...
async function buscarAlunos(paginaAtual) {
    ...
    const resposta = await fetch(`/api/usuarios/aluno/paginado?page=${paginaAtual}` + Filters.buildQueryString(), {
        method: 'GET',
        headers: {
...
async function deletarAluno(id) {
    const respostaDelete = await fetch(`/api/usuarios/aluno/${id}`, {
        method: "DELETE",
...
async function buscarNivel(selectElement) {
    ...
    const resposta = await fetch("/api/nivel-ingles", {
        method: 'GET',
        headers: {
...
async function atualizarNivel(id) {
    ...
    try {
        var resposta = await fetch(`/api/usuario-nivel-ingles/${id}`, {
            method: "PUT",
            body: id_nivel,
            headers: { 'Authorization': `Bearer ${token}`, "Content-type": "application/json; charset=UTF-8" }
        });
...
async function atualizarNicho(id) {
    ...
    try {
        var resposta = await fetch(`/api/usuario-nicho/${id}`, {
            method: "PUT",
            body: id_nicho,
            headers: { 'Authorization': `Bearer ${token}`, "Content-type": "application/json; charset=UTF-8" }
        });
